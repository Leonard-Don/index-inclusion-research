...
本文档审计当前 L3（中证官方候选名单）数据覆盖与继续扩展的可行路径。

> **首要结论（识别失败，先于数据覆盖）**：即便把 L3 扩到 ≥10 年，HS300 也**在数学上不构成 RDD**。
> 中证官方只发布"调入名单 / 备选名单"（排名序），不发布连续的内部 ranking score；数据里的
> `running_variable` 是按名单序号人工铺出的**等距数列**（实测 299.85, 299.86 … 300.28，间隔恒为 0.01），
> 与处理变量**100% 共线**（`inclusion == (running_variable>300)` 在 356 行中成立率 100%）、断点两侧
> **零重叠**（调入组 300.01–300.28、对照组 299.85–299.99）。因此 RDD 的连续性 / 不可精确操纵假设
> **根本无从陈述**，McCrary p≈0.44 只是等距构造的假象、无识别含义，placebo +0.05 反而 p≈0.116（假断点
> 都接近显著，是警告信号）。**这是设计层面的问题，不是数据量问题——补数据救不回。** 下面的数据覆盖审计
> 仅用于记录"即使想扩样本也走不通"这一附带事实；论文里 HS300 RDD 永久定位为附录里"尝试过但识别失败"的设计。
...
> 注意：下列条件即使全部满足，**也不足以让 HS300 成为有效 RDD**——因为 running variable 是名单序号
> 铺出的等距数列、与处理 100% 共线（见文首"首要结论"）。本节只记录"假如要继续扩样本，缺口在哪"。
...
- **额外要求**：每批次的 `running_variable` 与 `cutoff` 必须可追溯到中证官方调整名单序，
  不是公开重建反推。**但即使追溯到官方名单序，它仍是名单序号、不是连续 ranking score**——
  这正是识别失败的根因，不是采集精度问题。
...
- HS300 RDD 在论文中永久定位为附录里 **illustrative / preliminary、"尝试过但识别失败"** 的设计，
  **不作主表、不作因果支柱**。这不是"等数据补齐就转正"——设计本身不成立（见文首"首要结论"）。
- 论文主线是 **描述性** announce vs effective 事件研究 + 横截面匹配回归（treatment 系数是
  **横截面 CAR 水平差，不是 DiD**：无 pre-period、无 event×time 交互、无平行趋势检验），
  详见 `docs/paper_outline.md` 中的"研究设计"章节。本文不主张因果识别。
...
| 2026-05-03 | **主表切换决策**：当前 L3 = 11 批次 / 5 年，仍 < ≥20 批次 / 10 年门槛。HS300 RDD 主表用法继续保持 **illustrative / preliminary**；论文正文按附录 / 方法论补充章节呈现。门槛不动；触发条件挂在外部档案（Wayback / 付费数据库）。 | §3 RDD 主表用法；§6 临时定位 | claude（`analysis_parameters.md` §3 样本边界不变）|
