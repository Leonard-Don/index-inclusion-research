...
2. `dashboard_application.py`
   应用级装配。负责把 shell、services、route views 和 Flask app 接起来。
...
`app entrypoint -> application assembly -> shell/services/runtime -> route factory -> route handlers -> templates`
...
`track` 侧又分成三层 collaborator：

- [dashboard_track_support_runtime.py](../src/index_inclusion_research/dashboard_track_support_runtime.py)
  放格式化、label、snapshot meta、table tier 这类通用支持能力。
- [dashboard_track_content_runtime.py](../src/index_inclusion_research/dashboard_track_content_runtime.py)
  放结果读取、RDD 状态、文献内容加载、saved result 读取。
- [dashboard_track_display_runtime.py](../src/index_inclusion_research/dashboard_track_display_runtime.py)
  放“如何把 track 结果变成最终展示内容”的逻辑，以及 `run_and_cache_*`。

`DashboardTrackRuntime` 本身现在更像一个薄 façade：
外部仍然调用 `load_literature_framework_result()`、`run_and_cache_analysis()` 这类方法，但内部职责已经拆开了。
...
- [dashboard_page_sections_runtime.py](../src/index_inclusion_research/dashboard_page_sections_runtime.py)
  放首页 section 级内容：
  `design / robustness / limits / home_context`。
...
- [tests/test_dashboard_application.py](../tests/test_dashboard_application.py)
...
如果只是改视觉或页面内容，通常不需要动 `application/factory/services` 这一层。优先在 `dashboard_home.py`、`dashboard_sections.py`、`dashboard_presenters.py` 和静态资源里完成更安全。
