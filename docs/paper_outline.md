...
事件研究结果呈现出一个清晰的时间格局：公告窗 CAR[-1,+1] 在中美两市场均高度显著（中国 +1.76%，t=4.93；美国 +1.84%，t=5.25），而生效窗 CAR[-1,+1] 在两市场均约为零且不显著（中国 +0.42%，t=0.93；美国 -0.14%，t=-0.51）。这一"公告窗强、生效窗弱"的格局与信息/认证渠道的预测一致。更进一步，两市场公告窗量级几乎相同（相差仅 0.08 个百分点），而需求压力渠道预测中美公告窗应因指数基金规模差异而出现可识别分化——数据中未见此分化，这一跨制度相似性本身是支持信息渠道的独立证据。
