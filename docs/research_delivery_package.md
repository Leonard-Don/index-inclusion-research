...
3. **稳健性证据**（`results/real_tables/robustness_*.csv`、`results/real_figures/parallel_trends_aar_*.png`）：逐日 AAR 平行性图（事前窗 [-20,-2] 处理组与对照组重合）、伪事件日 placebo（US 公告窗 p=0.005、所有生效窗 p>0.29）、事件级置换检验（US 公告窗 p=0.0002、所有生效窗 p>0.27）、事件聚类 SE（按 event_date 聚类后 US 公告窗 p=0.0003 仍显著）互相印证"公告窗强、生效窗弱"。检验在匹配子样本上以事件窗内具备有效异常收益的事件计算，用于约束描述性结论，不把它包装成因果识别。
