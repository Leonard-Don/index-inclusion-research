...
- **参数记录日期**：2026-05-31（最近一次更新；见 §7 变更日志）
- **基线快照**：[`snapshots/pre-registration-2026-05-31.csv`](../snapshots/pre-registration-2026-05-31.csv)
  （方法学修正后重打的当前基线；CSV/CLI 名称为历史命名，保留以兼容 `--vs-pap` / `--compare-with` 工具；语义见 §6）
- **比对命令**：`index-inclusion-verdict-summary --compare-with snapshots/pre-registration-2026-05-31.csv`
...
- **样本**：`results/real_tables/cma_ar_path.csv`、`cma_gap_summary.csv`，n=455（CN+US）
- **统计量**：CN-US pre-runup 差异的置换检验 p-value（按 `announce_date` 聚类置换标签；bootstrap 仅用于区间估计 CI）
- **判据**：置换 p < 0.05 → 支持；0.05 ≤ p < 0.10 → 部分支持；p ≥ 0.10 → 证据不足
- **当前裁决**：证据不足（置换 p = 0.965，CI95=[-3.22%, 3.96%]）
...
- **当前裁决**：支持（双通道命中率 = 0.500）

### H4 · 卖空约束 — `evidence_tier=supplementary`

- **样本**：`cma_gap_event_level.csv` 控制 gap_length_days，n=455
- **统计量**：CN-US gap_drift 差异在事件级回归下的 p-value（HC3 标准误）
- **判据**：regression p < 0.05 → 支持；0.05 ≤ p < 0.10 → 部分支持；p ≥ 0.10 → 证据不足
- **当前裁决**：证据不足（regression p = 0.604，CN coef = 0.50%）

### H5 · 涨跌停限制 — `evidence_tier=core`

- **样本**：CN 事件级 `cma_h5_limit_predictive_regression.csv`，n=1096
- **统计量**：limit_coef 在 announce-day CAR 回归中的 p-value
- **判据**：limit_coef p < 0.05 → 支持；0.05 ≤ p < 0.10 → 部分支持；p ≥ 0.10 → 证据不足
- **当前裁决**：证据不足（limit_coef p = 0.427，limit_coef = 0.0744，R²=0.003）。本裁决反映已授权 Tushare A 股行情重算口径；Yahoo 时期的"支持/p=0.008"已被取代（数据驱动翻转，与 docs/limitations.md 及论文摘要一致）。

### H6 · 指数权重可预测性 — `evidence_tier=supplementary`

- **样本**：`cma_h6_weight_explanation.csv` + `cma_heterogeneity_size.csv`，匹配后 n=87
- **统计量**：heavy-light spread（announce_jump 中位数）+ OLS / sector-FE / quantile 三维稳健性
- **判据**：spread > 0 且至少两条稳健性回归 p < 0.10 → 支持；其余 → 证据不足
- **当前裁决**：证据不足（heavy-light spread = -0.016；OLS coef = -0.0057，p = 0.001 但方向反向）
...
- **当前裁决**：支持（US sector spread = 5.97，Materials +3.92 vs Real Estate -2.05）
...
| 2026-05-31 | 方法学修正（置换检验 / 聚类稳健性列 / 功效感知置信 / RDD 守卫）+ 重打基线 | 四项统计修正：(1) H1 pre-runup 由"各组绕自身均值 bootstrap、数零穿越"改为 H₀ 下按 `announce_date` 聚类的**置换检验**，boot_p 1.000→0.965（verdict 仍"证据不足"）；(2) event-study summary 新增 `se_car_clustered` / `p_value_clustered` 事件日聚类（CRV1，statsmodels）**稳健性列**，iid 列仍为主表口径；(3) **功效感知降置信**：事后 power<0.5 的假说 confidence 上限压到"低"并在 evidence_summary 标注 underpowered（H3 由"高"→"低"，verdict 仍"支持"；H4 标注 power=0.08），verdict 词不变；(4) RDD `fit_local_linear_rdd` 增退化窗口（n≤参数数）除零守卫，真实 spec 结果不变。重打 `snapshots/pre-registration-2026-05-31.csv` 为新裁决基线，pap-diff 相对该基线全部 unchanged。**历史 Yahoo→Tushare 的 H5/H2 翻转仍在摘要 / §5 / §7 与 limitations 散文、以及上方 2026-05-29 条目中披露**——本次重打基线不抹除该披露，仅把 deviation 比较基准重置到方法学修正后的当前状态。 | §2 H1 统计量与裁决；事件研究稳健性列；CMA confidence 口径；裁决基线快照 | leo | 本提交 |
