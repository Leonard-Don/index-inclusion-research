...
def build_sample_design_view(
    *,
    ...
    return {
        ...
        "detail_figures_title": f"展开其余样本设计图表（{detail_figures_count} 张）",
        "detail_figures_copy": "展示版默认收起其余样本图表，按需展开即可。",
        "primary": build_table_primary_view(
            ...
            collapsed_copy="展示版默认先显示样本范围总表，其余主表按需展开。",
        ),
        "detail": build_table_detail_view(
            full_title="补充细表",
            full_copy="这些表用于补充年份分布、数据来源与样本细节，适合在问答或写作时回查。",
            demo_key="demo-design-detail-tables",
            demo_title=f"展开样本设计补充表（{detail_tables_count} 张）",
            demo_copy="展示版默认收起年份分布和来源细表，问答时再展开。",
            container="library-panels",
        ),
    ...
    *,
    ...
    return {
        ...
        "primary": build_table_primary_view(
            ...
            collapsed_copy="展示版默认只显示一张主表，其余主表按需展开。",
        ),
        "detail": build_table_detail_view(
            full_title="补充细表",
            full_copy="这些表保留更细的变量拆解和辅助比较，用于支撑主结论。",
            demo_key=f"demo-{anchor}-detail-tables",
            demo_title=f"展开 {title} 的补充细表（{detail_tables_count} 张）",
        ...
        "support_demo_copy": "展示版默认收起支撑文献，需要追溯来源时再展开。",
