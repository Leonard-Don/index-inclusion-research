"""Static literature registry: 16 papers + camp / track / deep-analysis maps.

Organized as a single private module so the canonical paper metadata —
``PAPER_LIBRARY`` and the ``CAMP_LABELS`` / ``TRACK_LABELS`` /
``DEEP_ANALYSIS`` lookup dicts — sits in one place. Adding a new paper
only requires editing this file.

The frame and markdown builders live in sibling modules
(``_frames``, ``_markdown``) and import from here.
"""
