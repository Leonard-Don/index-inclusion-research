...
The descriptive claim ("announce-window strong, effective-window null") is backed by four independent checks, summarized in the committed [results/real_tables/robustness_event_study_summary.csv](results/real_tables/robustness_event_study_summary.csv) table. The full pipeline also regenerates the detailed `robustness_car_permutation.csv` family and `parallel_trends_aar_*.png` files when the raw matched panel is refreshed:
