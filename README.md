...
![CLI](https://img.shields.io/badge/CLI-41%20commands-2da44e)
...
4. [docs/cli_reference.md](docs/cli_reference.md)：41 个 console scripts 的完整说明。
...
- 总入口：`rebuild-all` / `verdict-summary` / `doctor` / `export-public-summary` / `paper-skeleton`（自动生成 `paper/skeleton.md`）/ `methodology-summary`（自动生成 `paper/methodology_summary.md` 方法论摘要卡）/ `citation-graph`（生成启发式文献关联网络 PNG/PDF/CSV，非逐条引用核验）/ `verdict-timeline`（从 git log 重建 H1-H7 裁决演进 swimlane 图）
