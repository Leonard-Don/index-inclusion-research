...
def test_reference_manifest_audit_normalizes_parent_directory_path_segments(
    tmp_path: Path,
) -> None:
    _seed_audit_project(tmp_path)
    status_path = tmp_path / "results" / "literature" / "hs300_rdd" / "rdd_status.csv"
    ...
    manifest_path = tmp_path / "results" / "real_tables" / "results_manifest.csv"
    manifest = pd.read_csv(manifest_path)
    manifest["rdd_source_file"] = "data/interim/../raw/hs300_rdd_candidates.csv"
    manifest["rdd_input_file"] = "data/raw/../raw/hs300_rdd_candidates.csv"
    manifest["rdd_audit_file"] = "results/literature/archive/../hs300_rdd/candidate_batch_audit.csv"
    manifest.to_csv(manifest_path, index=False)
