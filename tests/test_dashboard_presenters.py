...
def test_nav_sections_for_modes() -> None:
    ...
    assert [item["anchor"] for item in brief] == [
        "overview",
        "design",
        "tracks",
        "limits",
...
def test_mode_tabs_for_mode_uses_url_builder_and_allowed_hashes() -> None:
    tabs = dashboard_presenters.mode_tabs_for_mode(
        "demo",
        lambda mode, anchor=None: (
            f"/?mode={mode}" if anchor is None else f"/?mode={mode}#{anchor}"
        ),
    )
...
def test_prepare_track_display_splits_tables_and_keeps_demo_note() -> None:
    ...
    display = dashboard_presenters.prepare_track_display(
        ...
        result_cards_by_analysis={
            "price_pressure_track": [{"label": "结果", "value": "1", "copy": "说明"}]
        },
...
def test_prepare_track_display_updates_identification_copy_for_reconstructed_status() -> (
    None
):
    display = dashboard_presenters.prepare_track_display(
        {
    ...
    assert (
        display["track_view"]["primary"]["key"]
        == "demo-identification_china_track-primary-tables"
    )
...
def test_prepare_supplement_display_assigns_section_view_contract() -> None:
    ...
    assert [item["label"] for item in display["primary_tables"]] == [
        "事件时钟",
        "机制链",
    ]
    assert [item["label"] for item in display["detail_tables"]] == [
        "冲击估算步骤",
        "冲击估算示例",
        "表达框架",
    ]
