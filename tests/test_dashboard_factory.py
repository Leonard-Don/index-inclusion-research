...
def test_register_dashboard_route_views_accepts_registration_map() -> None:
    app = create_dashboard_app(
        import_name=__name__,
        template_folder="/tmp/templates",
        static_folder="/tmp/static",
    ...
    assert any(rule.endpoint == "show_paper_brief" for rule in app.url_map.iter_rules())
...
def test_build_dashboard_shell_wires_runtime_refresh_and_app() -> None:
    ...
    assert shell.app.static_folder.endswith("/tmp/example/scripts/static")
